...
{
  return runInInjectionContext(this.inject, () => {
  const collRef=collection(this.firestore,`notifs`);
 addDoc(collRef,{user,postId:postId,message:message,type:type,notifId:notifId,date:new Date()});
  })
}
 ...
 sendFrindRequstNotif(user:any,message:string,notifId:string) // الشخص ال هيروحله الاشعار 
{
  return runInInjectionContext(this.inject, () => {
  const collRef=collection(this.firestore,`notifs`);
   addDoc(collRef,{user,message:`${user.name} ${message}`,type:'request',notifId:notifId,date:new Date()});
  })
